...
let newArray = mixNumbers.filter(function(element, index, array)){
  return element % 2 === 0;
}
...
let mapArray = prices.map(function(element, index, array)){
  return element *= 1.07;
}
...
let newPop = populations.reduce(function(acc, curr)){
  return element acc + curr;
}
...
let newMonsters = monstersInYourPocket.filter(function(element, index, array)){
  if(element > 200){
    return element
  }
}
